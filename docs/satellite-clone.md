...
#### Important Notes ####
  ...
  - If you are using NFS for storage and your pulp backup tar file is large (>150 gb), you might see memory errors while untaring pulp data.  In this case you can optionally choose to skip pulp restore by setting `skip_pulp_data` to `true` in `satellite-clone-vars.yml` and copying /var/lib/pulp over to the machine manually.
